...
if (isset($_SESSION['role']) && isset($_SESSION['id'])) {
    if (isset($_POST['title']) && isset($_POST['description']) && $_SESSION['role'] == 'admin' && isset($_POST['due_date'])) {
        ...
        $member_ids = array_filter(array_map('intval', $member_ids), function ($id) {
            return $id > 0;
        });
        ...
        if (empty($title)) {
            ...
            if ($assignment_mode === 'group') {
                ...
                foreach ($members as $m) {
                    if ($m['role'] === 'member' && $m['user_role'] !== 'admin') {
            ...
            // Auto-create a group chat for this task (admin + leader + members)
            $created_by = (int)$_SESSION['id'];
            $chat_member_ids = $member_ids;
            // Explicitly include admin in the chat group so they can communicate with the team
